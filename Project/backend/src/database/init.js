...
db.serialize(() => {
  ...
  roleStmt.run(2, 'editor', '编辑者', JSON.stringify(['document:read', 'document:write', 'document:delete']));
  ...
  const userStmt = db.prepare("INSERT OR IGNORE INTO users (username, email, password_hash, full_name, role_id, status) VALUES (?, ?, ?, ?, ?, ?)");
  ...
  // 插入基础标签
  const tagStmt = db.prepare("INSERT OR IGNORE INTO tags (name, color, description) VALUES (?, ?, ?)");
